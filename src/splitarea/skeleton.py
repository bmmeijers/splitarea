...
#         """
    ...
    for he in external:
        ...
        # print "adding outside edge", he
        eid, sn, en, lf, rf, geom, = he
#         eid, geom, sn, sn_ext, svtxid, en, en_ext, evtxid, lf, rf, = he.id, he.geometry, \
#             he.start_node.id, False, he.start_node.id, \
#             he.end_node.id, False, he.end_node.id, \
#             he.left_face.id, he.right_face.id
...
#             print v0, v1
        ...
        if start_node_id is None:
            start_node_id = (id(v0), ) # note, not an int but tuple to prevent duplicate with external ids
        end_node_id = v1.info.vertex_id
        if end_node_id is None:
            end_node_id = (id(v1), ) # note, not an int but tuple to prevent duplicate with external ids
...
def label_sides(skeleton):
    ...
    for he in external:
        ...
        while he.next.attrs['external'] == False:
            he = he.next
            assert he.face.id is None # the face id currently should be None
...
#     print "labelled edges\n=================="
#     for he in skeleton.half_edges.itervalues():
#         print he.id, he.face, he.twin.face

# FIND FINAL EDGES

# find groups of half edges to be glued together to form longer chains
# we glue edges in both directions, using he.next and he.prev
#
# FIXME: longer chains can also be formed by remaining edges!
#
# Maybe better: start at arbitrary edge,
# glue edges as long as:
# we cross a degree == 2 node
# and the left / right remains the same
# and we should not cross an external edge either
#
...
#         he = start
#         if start.id not in seen:
#             seen.add(he.id)
#             group = [he]
#             # as long as:
#             # - the faces are the same we glue the edge
#             # - this is not the start edge
#             # - and we have found another external edge
#             while he.next.face is start.face and \
#                 he.next.twin.face is start.twin.face and \
#                 he.next is not start:
#                 # we go to the next edge and add it to the group
#                 he = he.next
#                 seen.add(he.id)
#                 group.append(he)
#                 # or until we find an other external edge
...
#             print he.id
            if he.anchor is not None:
                ...
                # take reverse geometry
                part = he.twin.anchor.geometry[::-1]
            if geom is None:
                # this is the first part of the new geometry 
                geom = part
            else:
                # take of last point and add the part to it
                geom.pop()
                geom.extend(part)
#         print "geom ", geom
#         print "left ", group[0].face.id
#         print "right", group[0].twin.face.id
#         print "start", group[0].origin.id
#         print "end  ", group[-1].twin.origin.id
        new_edge = (new_edge_id, 
                    group[0].origin.id, group[-1].twin.origin.id,
